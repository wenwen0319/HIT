    ...
    with torch.no_grad():
        ...
        for k in tqdm(range(loop_num)):
            ...
            if interpretation:
                ...
                # src_1_walks_score = torch.softmax(src_1_walks_score, 1).detach().cpu().numpy()
                # src_2_walks_score = torch.softmax(src_2_walks_score, 1).detach().cpu().numpy()
                # tgt_walks_score = torch.softmax(tgt_walks_score, 1).detach().cpu().numpy()
                ...
                # print(src_1_l_cut.shape, src_1_walk_pattern.shape)
                # calculate the ratio
                # print(true_label.shape, true_label,type(true_label))
                ...
                # time_list for distribution
                # time_list = [_temp, t.view(-1)]
