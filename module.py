...
class HIT(torch.nn.Module):
    ...
    def forward(self, src_idx_l_1, src_idx_l_2, tgt_idx_l, cut_time_l, subgraphs=None, time_gt=None):
        ...
        if self.pos_dim > 12: # when pos_dim we consider there is no DE
...
class PositionEncoder(nn.Module):
    ...
    def __init__(self, num_layers, enc='spd', enc_dim=2, ngh_finder=None, verbosity=1, cpu_cores=1, logger=None):
        ...
        elif self.enc in ['concat'] :
            # assert(self.enc in ['concat'])
                ...
                for idx_batch, batch in enumerate(walk_encodings_cpu):
                    for idx_walk, walk in enumerate(batch):
                        if interpretation == 1:
                            """
                            symmetric
                            for interpretation 1(triangle vs closure, they are symmetric)
                            """
                            ...
                            """
                            asymmetric
                            for interpretation 2, 3(wedge are asymmetric)
                            """
                            walk_1 = np.array2string(walk)
                            if walk_1 in self.pattern:
                                walk = walk_1
