...
class NeighborFinder:
    def __init__(self, adj_list, bias=0, ts_precision=PRECISION, use_cache=False, sample):
        """
        Params
        ------
        node_idx_l: List[int]
        ...
        node_idx_l, node_ts_l, edge_idx_l, off_set_l, self.nodeedge2idx = self.init_off_set(adj_list)
    ...
    def init_off_set(self, adj_list):
        ...
        return n_idx_l, n_ts_l, e_idx_l, off_set_l, nodeedge2idx
    ...
    def find_before(self, src_idx, cut_time, e_idx=None):
        ...
        if e_idx is None:
            cut_idx = bisect_left(neighbors_ts, cut_time)  # very crucial to exempt ties (so don't use bisect)
        else:
    ...
    def get_temporal_neighbor(self, src_idx_l, cut_time_l, num_neighbor=20, e_idx_l=None):
        ...
        for i, (src_idx, cut_time) in enumerate(zip(src_idx_l, cut_time_l)):
            ngh_idx, ngh_eidx, ngh_ts = self.find_before(src_idx, cut_time, e_idx=e_idx_l[i] if e_idx_l is not None else None)
