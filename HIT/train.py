...
def train_val(dataset, model, mode, bs, epochs, criterion, optimizer, early_stopper, ngh_finders, logger, interpretation=False):
    ...
    for epoch in range(epochs):
        ...
        val_acc, val_ap, val_f1, val_auc, cm = eval_one_epoch('val for {} nodes'.format(mode), model, dataset, val_flag='val',interpretation=interpretation)
        # print("Confusion matrix\n", cm)
        logger.info('confusion matrix: ')
        logger.info(', '.join(str(r) for r in cm.reshape(1,-1)))
        model.update_ngh_finder(full_ngh_finder)
        val_acc_t, val_ap_t, val_f1_t, val_auc_t, cm = eval_one_epoch('val for {} nodes'.format(mode), model, dataset, val_flag='test',interpretation=interpretation)
        # print("Confusion matrix\n", cm)
        logger.info('confusion matrix: ')
        logger.info(', '.join(str(r) for r in cm.reshape(1,-1)))
