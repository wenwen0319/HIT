...
file_name = 'tags-ask-ubuntu'
# file_name = 'DAWN'
# file_name = 'congress-bills'
# file_name = 'email-Eu'
# file_name = 'tags-math-sx'
...
for i in fin_nverts:
    nverts.append(int(i))
count = 0 # note nodes start from 0
...
output_hyperedge_e = {}
output_hyperedge_t = {}
output_hyperedge_v = set(np.arange(count + 1))
for idx_y in y:
    node_list = node_list_total[idx_y]
    if len(node_list) == 1:
        continue
    simplex_idx += 1
    output_hyperedge_e[simplex_idx] = [set(node_list), set(node_list)]
    output_hyperedge_t[simplex_idx] = times[idx_y]
...
output_hyperedge = {}
output_hyperedge['V'] = output_hyperedge_v
output_hyperedge['E'] = output_hyperedge_e
output_hyperedge['t'] = output_hyperedge_t
import pickle
with open('./NHP_preprocess/'+file_name + '_hypergraph.pkl', 'wb') as handle:
    pickle.dump(output_hyperedge, handle, protocol=pickle.HIGHEST_PROTOCOL)
