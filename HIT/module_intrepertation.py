...
class finalClassifier_time_prediction(torch.nn.Module):
    ...
    def __init__(self, dim1, dim2, dim3, dim4, dim5, N=3):
        ...
        # # self.MLP_mean = torch.nn.Linear(dim1, dim4)
        ...
        self.N = N
        # self.MLP_three_nodes = torch.nn.Linear(dim1 + dim2 + dim3, dim4)
        self.fc_mu = nn.Sequential(nn.Linear(dim4, dim4), nn.ReLU(), nn.Linear(dim4, N))
        # self.fc_var = nn.Linear(dim1 + dim2 + dim3, dim4)
        ...
        # self.fc_mu = nn.Sequential(nn.Linear(dim1 + dim2 + dim3, dim4), nn.ReLU(), nn.Linear(dim4, N))
        # # self.fc_var = nn.Linear(dim1 + dim2 + dim3, dim4)
        # self.fc_log_var = nn.Sequential(nn.Linear(dim1 + dim2 + dim3, dim4), nn.ReLU(), nn.Linear(dim4, N)) # because may <0 so add exp to make sure it is greater than 0
        # self.fc_weight = nn.Sequential(nn.Linear(dim1 + dim2 + dim3, dim4), nn.ReLU(), nn.Linear(dim4, N), nn.Softmax())
        ...
        # torch.nn.init.xavier_normal_(self.fc_weight.weight)
        ...
        # self.MLP = torch.nn.Linear(dim4, N * 3)
        # self.MLP_mean = torch.nn.Linear(dim1, dim4)
        # self.MLP_last = torch.nn.Linear(dim4, dim4)
    ...
    def forward(self, x1, x2, x3, t):
        ...
        # f = self.fc2(self.act(self.MLP_last(h)))
        ...
        # prec = torch.sqrt(var) # std
        prec = torch.sqrt(var * 2 * np.pi) # std
        weight = self.fc_weight(f).view(-1, self.N)

        """
        update in 2021.02.01
        TODO: merge log_p and result into logsumexp
        """
        # log_p = torch.exp( - (mu * mu + t * t - 2 * t * mu) / (prec ** 2 * 2)) / (torch.sqrt(2 * self.pi) * prec) * weight
        ...
        result = - torch.log(torch.sum(log_p, dim=1))
        if self.sum_data:
            return MSE, torch.sum(result)
        else:
            return MSE, result
        """
        # log_det = torch.sum(torch.log(prec), dim=2, keepdim=True)

        # log = -.5 * (self.n_features * np.log(2. * np.pi) + log_p) + log_det
        # a = (t - mu) ** 2
        # log_p = -0.5 * (logvar + a / logvar.exp())
        
        # log_p = log_p + self.log_norm_constant

        # log_likelihoods = log_p.sum(-1)

        # e = torch.exp(- ((t - mu).t() * torch.pow(std, -2) * (t-mu) / 2))
        # log_det = torch.sum(torch.log(std), dim=1, keepdim=True)
        # NLL = - torch.log(torch.sum(weight * e / (torch.sqrt(2 * np.pi) * std), dim=0))
        # log_prob = batch_size * np.log(2. * np.pi) + log_p + log_det - torch.log(weight) # already negative, just minimize

        # weighted_log_prob = log_prob + torch.log(weight)
        # per_sample_score = NLL # torch.logsumexp(log_prob, dim=1)
        # # aim to maximize loglikelihood, so minimize negative loglikelihood
        # if self.sum_data:
        #     return per_sample_score.sum()
        # else:
        #     return torch.squeeze(per_sample_score)
        # f = self.MLP(self.act(self.MLP_three_nodes(f_uw)))
        # f_vw = self.act(self.MLP_two_nodes(f_vw))
        # h = f_uw + f_vw
        ...
        # MLP(Mean(u, v, w))
        # """
        # h = self.MLP_last(self.act(self.MLP_mean(x1 + x2 + x3)))
...
class finalClassifier_inter(torch.nn.Module):
    def __init__(self, dim1, dim2, dim3, dim4, dim5):
        ...
        # self.MLP_two_nodes = torch.nn.Linear(dim1, dim4) #TODO: No need, to be delete
        # self.MLP_last = torch.nn.Linear(dim4, dim4) # TODO: No need, to be delete
    ...
    def forward(self, x1, x2, x3):
        ...
        # else:
        #     h = self.fc2(self.act(self.MLP_last((x1+x2+x3) / 3)))
...
class finalClassifier(torch.nn.Module):
    ...
    def forward(self, x1, x2, x3):
        ...
        # h = self.fc2(self.act(self.MLP_last(self.act(self.MLP_two_nodes(torch.cat([x1+x2,x3], dim=-1))))))
...
class CAWN(torch.nn.Module):
    ...
    def contrast(self, src_idx_l_1, src_idx_l_2, tgt_idx_l, cut_time_l_pos, e_idx_l_pos, endtime_pos=None):
        ...
        score, walk_bucket = self.forward(src_idx_l_1, src_idx_l_2, tgt_idx_l, cut_time_l_pos, (subgraph_src_1_pos, subgraph_src_2_pos, subgraph_tgt), time_gt=endtime_pos)
        ...
        # print(score)
        return score, walk_bucket # original sigmoid, because we use CrossEntropyLoss, so no softmax
        # return torch.nn.functional.softmax(score)

    def forward(self, src_idx_l_1, src_idx_l_2, tgt_idx_l, cut_time_l, subgraphs=None, time_gt=None):
        ...
        src_1_walk_pattern, src_embed_1 = self.forward_msg(src_idx_l_1, cut_time_l, subgraph_src_1)
        src_2_walk_pattern, src_embed_2 = self.forward_msg(src_idx_l_2, cut_time_l, subgraph_src_2)
        tgt_walk_pattern, tgt_embed = self.forward_msg(tgt_idx_l, cut_time_l, subgraph_tgt)
        # # if self.agg == 'walk' and self.walk_mutual:
        # #     src_embed_1, src_embed_2, tgt_embed = self.tune_msg(src_embed_1, src_embed_2, tgt_embed)
        # # print(src_embed_1.size())
...
class PositionEncoder(nn.Module):
    ...
    def __init__(self, num_layers, enc='spd', enc_dim=2, ngh_finder=None, verbosity=1, cpu_cores=1, logger=None):
        ...
        elif self.enc in ['sym'] :
            assert(self.enc in ['sym'])
            self.trainable_embedding_distance = nn.Sequential(nn.Linear(in_features=3 * (self.num_layers+1), out_features=self.enc_dim), nn.ReLU(),
                                                                    nn.Linear(in_features=self.enc_dim, out_features=self.enc_dim), nn.ReLU())
            # self.trainable_embedding_distance_agg = nn.Sequential(nn.Linear(in_features=self.enc_dim, out_features=self.enc_dim), nn.ReLU(),
            #                                                       nn.Linear(in_features=self.enc_dim, out_features=self.enc_dim), nn.ReLU(),
            #                                                     #   nn.Linear(in_features=self.enc_dim, out_features=self.enc_dim), nn.ReLU(),
            #                                                       )
        elif self.enc in ['sum_pooling','sum_pooling_after']:
            self.trainable_embedding_distance = nn.Sequential(nn.Linear(in_features=(self.num_layers+1), out_features=self.enc_dim), nn.ReLU(),
                                                              nn.Linear(in_features=self.enc_dim, out_features=self.enc_dim), nn.ReLU())
    ...
    def collect_pos_mapping_ptree_sample(src_1, src_2, tgt, cut_time, src_neighbors_node_1, src_neighbors_ts_1, src_neighbors_node_2, src_neighbors_ts_2,
        ...
        elif enc in ['sym', 'sum_pooling', 'sum_pooling_after']:
            # landing probability encoding, n_hop+1 types of probabilities for each node
            # src_neighbors_node, src_neighbors_ts = [[src]] + src_neighbors_node, [[cut_time]] + src_neighbors_ts
            src_neighbors_node_1, src_neighbors_ts_1 = [[src_1]] + src_neighbors_node_1, [[cut_time]] + src_neighbors_ts_1 
            src_neighbors_node_2, src_neighbors_ts_2 = [[src_2]] + src_neighbors_node_2, [[cut_time]] + src_neighbors_ts_2
    ...
    def forward(self, node_record, t_record, test, interpretation):
        ...
        if self.enc != 'saw':
            ...
            # walk_encodings: batch, walk_size, walk_length, for (u,v,w), position_encoding_dim(3 for spd, walk_length for lp)
            # """
            walk_pattern = None
            if (test) and (interpretation>0):
                ...
                for idx_batch, batch in enumerate(walk_encodings_cpu):
                    for idx_walk, walk in enumerate(batch):
                        if interpretation == 8:
                            """symetric"""
                            # print(walk)
                            walk_1 = np.array2string(walk)
                            walk_2 = np.copy(walk)
                            walk_2[:, [0, 1]] = walk_2[:, [1, 0]]
                            walk_2 = np.array2string(walk_2)
                            if walk_1 in self.pattern:
                                walk = walk_1
                            elif walk_2 in self.pattern:
                                walk = walk_2
                            else:
                                # if walk_1 not in self.pattern:
                                walk = walk_1
    ...
    def get_trainable_encodings(self, encodings):
        ...
        if self.enc == 'spd':
            ...
            v = encodings[:,:,1,:]
            w = encodings[:,:,2,:]
            encodings = self.trainable_embedding_distance(torch.cat([u,w,v], 2))
            # d_vw = self.trainable_embedding_distance(torch.cat([v,w], 2))
            # encodings = self.trainable_embedding_distance_agg(torch.cat([d_uw+d_vw, torch.abs(d_uw-d_vw)], 2))
        elif self.enc == 'sum_pooling':

            encodings = encodings.float()
            u = encodings[:,:,0,:]
            v = encodings[:,:,1,:]
            w = encodings[:,:,2,:]
            encodings = self.trainable_embedding_distance(u+w+v)
            # d_vw = self.trainable_embedding_distance(torch.cat([v,w], 2))
            # encodings = self.trainable_embedding_distance_agg(torch.cat([d_uw+d_vw, torch.abs(d_uw-d_vw)], 2))
        elif self.enc == 'sum_pooling_after':

            encodings = encodings.float()
            u = encodings[:,:,0,:]
            v = encodings[:,:,1,:]
            w = encodings[:,:,2,:]
            encodings = self.trainable_embedding_distance(u)+self.trainable_embedding_distance(w)+self.trainable_embedding_distance(v)
        else:
            assert(self.enc == 'saw')
